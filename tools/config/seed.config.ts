...
export class SeedConfig {
  ...
  */
  ...
  /**
   * The folder of the applications less files.
   * @type {string}
   */
  LESS_SRC = `${this.APP_SRC}/less`;
  ...
  /**
   * Enable LESS stylesheet compilation.
   * Set ENABLE_LESS environment variable to 'true' or '1'
   * @type {boolean}
   */
  ENABLE_LESS = ['true', '1'].indexOf(`${process.env.ENABLE_LESS}`.toLowerCase()) !== -1 || argv['less'] || false;
  ...
  */
  ...
  getInjectableStyleExtension() {
    return (this.BUILD_TYPE === BUILD_TYPES.PRODUCTION && this.ENABLE_LESS) ? 'scss' : ((this.BUILD_TYPE === BUILD_TYPES.PRODUCTION && this.ENABLE_SCSS) ? 'scss' : 'css');
  }
