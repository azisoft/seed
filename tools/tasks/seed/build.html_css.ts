...
const appLESSFiles      = join(Config.APP_SRC, '**', '*.less');
const entryLESSFiles    = join(Config.CSS_SRC, '**', '*.less');
const abtractLESSFiles  = join(Config.LESS_SRC, '**', '*.less');
...
function processComponentStylesheets() {
  // return Config.ENABLE_LESS ?
  //   merge(
  //     processComponentLess(),
  //     processComponentCss())
  //   :  Config.ENABLE_SCSS ?
  //   merge(
  ...
  //   processComponentCss();
  return merge(
      Config.ENABLE_SCSS ? processComponentScss() : gulp.src([]),
      Config.ENABLE_LESS ? processComponentLess() : gulp.src([]),
      processComponentCss());
}
...
/**
 * Process LESS files referenced from Angular component `styleUrls` metadata
 */
function processComponentLess() {
  return getLESSFiles('process-component-less', [appLESSFiles], [abtractLESSFiles])
    .pipe(plugins.sourcemaps.init())
    .pipe(plugins.less().on('error', reportPostCssError))
    .pipe(plugins.postcss(processors))
    .on('error', reportPostCssError)
    .pipe(plugins.sourcemaps.write(isProd ? '.' : '', {
      sourceMappingURL: (file: any) => {
...
/**
 + * Get LESS Files to process
 + */
function getLESSFiles(cacheName:string, filesToCompile:string[], filesToExclude:string[] = []) {
  let allFiles:string[] = filesToCompile.concat(filesToExclude);
  let filteredFiles:string[] = filesToCompile.concat(
    filesToExclude.map((path:string) => { return '!' + path; })
  );
...
/**
 * Get a stream of external LESS files for subsequent processing.
 */
function getExternalLessStream() {
  return getLESSFiles('process-external-less', getExternalLess(), [abtractLESSFiles])
    .pipe(plugins.less().on('error', reportPostCssError));
}

/**
 * Get an array of filenames referring to external LESS stylesheets located in the global DEPENDENCIES
 * as well as in `src/css`.
 */
function getExternalLess() {
  return Config.DEPENDENCIES.filter(dep => /\.less$/.test(dep.src)).map(dep => dep.src)
    .concat([entryLESSFiles]);
}
